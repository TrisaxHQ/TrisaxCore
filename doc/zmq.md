...
The Trisax Core daemon can be configured to act as a trusted "border
router", implementing the trisax wire protocol and relay, making
...
The ZeroMQ feature in Trisax Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building trisaxd:
...
For instance:
    ...
    $ trisaxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/trisaxd.tx.raw \
...
These options can also be provided in trisax.conf.
...
From the perspective of trisaxd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into trisaxd directly. Furthermore, no information is
...
using. Trisaxd appends an up-counting sequence number to each
