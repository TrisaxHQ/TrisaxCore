Trisax Core version 0.18.1 is now available from:

  <https://download.trisax.org/trisax-0.18.1/>
...
`/Applications/Trisax-Qt` (on Mac) or `trisaxd`/`trisax-qt` (on
Linux).
...
Trisax Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Trisax Core on unsupported systems.
...
10.10. Additionally, Trisax Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:18877:18877` (this is an extra `:18877` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@trisax.org) the Trisax Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 TRSX),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in TRSX per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/trisax-conf.md)
  about the `trisax.conf` file describes how to use it to configure
  Trisax Core.

- A new document introduces Trisax Core's BIP174 [Partially-Signed
  Trisax Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Trisax-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Trisax Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Trisax Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Trisax Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart trisaxd with the `-deprecatedrpc=generate`
...
- A new `trisax-wallet` tool is now distributed alongside Trisax
...
This section describes planned changes to Trisax Core that may affect
other Trisax software and services.

- Since version 0.16.0, Trisax Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Trisax Core 0.20 (expected about
  a year after 0.18), Trisax Core will default to native segwit
  ...
  to bech32 addresses, and if the Trisax Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Trisax Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Trisax Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Trisax Core
...
- A sub-project of Trisax Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Trisax Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Trisax Core's own implementation, although entropy
  gathered by Trisax Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Trisax Core a
...
- On macOS, Trisax Core now opts out of application CPU throttling
...
- #18877 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
